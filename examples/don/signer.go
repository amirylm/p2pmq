package don

import (
	"bytes"
	"crypto/sha256"
	"fmt"
)

type Signer interface {
	OracleID() OracleID
	Sign(reportCtx ReportContext, report []byte) ([]byte, error)
	Verify(pubKey OnchainPublicKey, reportCtx ReportContext, report, signed []byte) error
}

// Sha256Signer is a mocked signer that uses `sha256(data)` as signature.
type Sha256Signer struct{}

func (s *Sha256Signer) OracleID() OracleID {
	return 0
}

func (s *Sha256Signer) Sign(_ ReportContext, report []byte) ([]byte, error) {
	h := sha256.Sum256(report)
	return h[:], nil
}

func (s *Sha256Signer) Verify(_ OnchainPublicKey, _ ReportContext, report, signed []byte) error {
	h := sha256.Sum256(report)
	if !bytes.Equal(signed, h[:]) {
		return fmt.Errorf("signature mismatch")
	}
	return nil
}
